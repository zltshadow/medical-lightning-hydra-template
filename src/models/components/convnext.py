...
from timm_3d.data import (
    IMAGENET_DEFAULT_MEAN,
    IMAGENET_DEFAULT_STD,
    OPENAI_CLIP_MEAN,
    OPENAI_CLIP_STD,
)
from timm_3d.layers import (
    trunc_normal_,
    AvgPool3dSame,
    DropPath,
    Mlp,
...
from timm_3d.layers import NormMlpClassifierHead, ClassifierHead
from timm_3d.models._builder import build_model_with_cfg
from timm_3d.models._manipulate import named_apply, checkpoint_seq
from timm_3d.models._registry import (
    generate_default_cfgs,
    register_model,
    register_model_deprecations,
)
from timm_3d.models import create_model
...
class Downsample(nn.Module):
    ...
    def __init__(self, in_chs, out_chs, stride=1, dilation=1):
        ...
        if in_chs != out_chs:
            self.conv = create_conv3d(in_chs, out_chs, 1, stride=1)
        else:
...
class ConvNeXtBlock(nn.Module):
    ...
    def __init__(
        ...
        dilation = to_ntuple(3)(dilation)
        act_layer = get_act_layer(act_layer)
        if not norm_layer:
            norm_layer = LayerNorm3d if conv_mlp else LayerNorm
        mlp_layer = partial(
...
class ConvNeXt(nn.Module):
    ...
    def __init__(
        ...
        dp_rates = [
            x.tolist()
            for x in torch.linspace(0, drop_path_rate, sum(depths)).split(tuple(depths))
        ]
        stages = []
        prev_chs = dims[0]
...
if __name__ == "__main__":
    ...
    model = _create_convnext(
        "convnextv2_base", pretrained=False, **dict(model_args)
    ).to("cuda")
    model = ConvNeXt(
        in_chans=1,
        num_classes=2,
